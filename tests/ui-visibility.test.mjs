...
for (const id of ['docs', 'viewer', 'loading', 'classPick']) {
  const el = byId[id];
...
/* ---- version stamp ---- */
check('app version is stamped in the markup', /name="app-version" content="2\.0\.1"/.test(html), true);
check('app version is stamped in app.js', /APP_VERSION = '2\.0\.1'/.test(appjs), true);
